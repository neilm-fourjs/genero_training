...
FUNCTION loadArray(l_node om.domNode)
	...
	FOR x = 1 TO l_nl.getLength()
		...
		END IF
		IF m_arr[x].dfv IS NULL THEN
			LET m_arr[x].dfv = "auto"
		END IF
		IF m_arr[x].cxt IS NULL THEN
			LET m_arr[x].cxt = "auto"
		END IF
	END FOR
		...
		DISPLAY ARRAY m_arr TO arr.*
			...
			ON DELETE
				IF fgldialog.fgl_winQuestion(
								"Confirm", "Delete action?", "No", "Yes|No", "question", 0)
						= "No" THEN
					LET int_flag = TRUE
				ELSE
					CALL DIALOG.setActionActive("save", TRUE)
