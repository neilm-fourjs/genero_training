...
FUNCTION set_dest(l_rptName)
	...
	MENU "Report Destination" ATTRIBUTES(STYLE = "dialog", COMMENT = "Output report to ...", IMAGE = "question")
		...
		COMMAND "Browser"
			LET m_dest    = "B"
			LET m_device  = "Browser"
			LET l_preview = TRUE
		COMMAND "Printer"
			LET m_dest   = "P"
			LET m_device = "Printer"
		COMMAND "PDF"
			LET m_dest    = "D"
			LET m_device  = "PDF"
			LET l_preview = TRUE
	...
	IF l_rptName IS NOT NULL THEN
		LET l_rptName = l_rptName.append(".4rp")
	END IF
	...
	IF m_dest = "F" THEN
		...
		IF m_fileName IS NULL THEN
			LET m_fileName = base.Application.getProgramName()
		END IF
	...
	IF m_device = "PDF" AND l_rptName IS NULL THEN
		CALL fgl_report_configureCompatibilityOutput(
				m_pageWidth, "Courier", TRUE, base.Application.getProgramName(), "", "")
	END IF
    ...
    IF m_gre IS NULL THEN 
        CALL fgl_winMessage("Error",SFMT("Failed to initialize report '%1'.", l_rptName), "exclamation")
        EXIT PROGRAM
    END IF
