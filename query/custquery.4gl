...
FUNCTION cust_select(p_where_clause)
	...
	LET sql_text = "SELECT * FROM customer " || " WHERE " || p_where_clause
...
FUNCTION inpupd_cust(au_flag)
	...
	INPUT BY NAME mr_custrec.* WITHOUT DEFAULTS ATTRIBUTE(UNBUFFERED)
		...
		ON CHANGE store_num
			IF au_flag = "A" THEN
				SELECT * INTO mr_custrec.* FROM customer WHERE store_num = mr_custrec.store_num
...
FUNCTION update_cust()
	DEFINE
		l_custrec RECORD LIKE customer.*,
		matches   INTEGER,
		cont_ok   INTEGER
	...
	SELECT * INTO l_custrec.* FROM customer
			WHERE store_num = mr_custrec.store_num -- FOR UPDATE -- Not compatible with SQLite
	...
	ELSE
		...
		IF matches = TRUE THEN
			TRY
				UPDATE customer
						SET store_name = mr_custrec.store_name, addr = mr_custrec.addr, addr2 = mr_custrec.addr2,
								city = mr_custrec.city, state = mr_custrec.state, zipcode = mr_custrec.zipcode,
								contact_name = mr_custrec.contact_name, phone = mr_custrec.phone, created_on = mr_custrec.created_on
...
FUNCTION delete_check()
	...
	SELECT COUNT(*) INTO del_count FROM orders WHERE orders.store_num = mr_custrec.store_num
